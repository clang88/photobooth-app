...
class FilterNanobanana(BaseFilter[FilterNanobananaConfig]):
    ...
    def _apply_gemini_filter(self, image: Image.Image, filter_type: str, preview: bool) -> Image.Image:
            ...
            try:
                ...
                for part in parts:
                    if "inlineData" in part:
                        ...
                        logger.warning(f"Gemini response contains text instead of image: {gemini_text_response[:200]}...")

                if generated_image is None:
                    reason = gemini_text_response or finish_reason
                    error_detail = "No image data received from AI"
                    if reason:
                        error_detail += f":\n{reason}"
                    logger.error(f"No image generated. finishReason={finish_reason}, parts={parts}")
                ...
                raise RuntimeError(f"Request to Gemini API timed out after {1 + max_retries} attempt(s): {e}") from e
        ...
        raise RuntimeError(f"Request to Gemini API failed after {1 + max_retries} attempt(s)") from last_exception

    def _add_error_text(self, image: Image.Image, error_message: str) -> Image.Image:
        """Overlay error text on the lower part of the fallback image."""
        draw = ImageDraw.Draw(image)
        width, height = image.size
