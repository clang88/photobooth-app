...
class FilterNanobananaConfig(BaseConfig):
    ...
    style_prompts: list[StylePrompt] = Field(
        default=[
            ...
            StylePrompt(
                style_name="6-vacation-2",
                prompt="Apply a light filter that overlays a semi-transparent collage of passport entry stamps from different countries (e.g., Austria, Italy, Spain, Thailand) across the image and some airplanes and trains, *avoiding* covering the people's faces. Keep the subjects and background unchanged.",
            ),
            StylePrompt(
                style_name="7-train",
                prompt="Put old-fashioned train conductors hats on all the people in the image and put a steam-powered train into the background. Keep their faces, poses, and expressions unchanged. Do not add or remove people!",
            ),
            StylePrompt(
                style_name="8-car",
            ...
            StylePrompt(
                style_name="25-gamers",
                prompt="Put all the people in this image into a 2D side-scroller beat 'em up video game scene. Make them all fight a godzilla with a bride veil. Keep them recognizable but draw them in a pixalated 8-bit style. Don't add names to the characeters, only the bridezilla healthbar should have a name.",
            ),
            StylePrompt(
                style_name="26-japan",
