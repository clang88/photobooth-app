...
def _empty_str_to_path(v: str | Path | None) -> Path | None:
    """Convert empty strings to None, validate non-empty paths exist as files."""
    if v is None:
        return None
...
FilePathOrNone = Annotated[Path | None, BeforeValidator(_empty_str_to_path)]
...
class CollageMergeDefinition(BaseModel):
    ...
    predefined_image: FilePathOrNone = Field(
        default=None,
